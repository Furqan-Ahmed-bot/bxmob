...
import '../DataLayer/Services/genericService.dart';
...
class _AuthenticationWrapperState extends State<AuthenticationWrapper> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      if (mounted) {
        ...
        if (user) {
          ...
          dynamic supportresult;
        supportresult = await SupportMenu();
        if (supportresult[0].ApiError.length > 0) {
            if (supportresult[0].ApiError['StatusCode'] == 401) {
              setState(() {
                navigateToSplash = true;
                isDataLoaded = false;
                somethingWrong = false;
          ...
          }  else if (supportresult[0].Data.length >= 0) {
            if (supportresult[0].Data.contains("[{") && supportresult[0].Data.length > 0) {
              supportData = Functions.convertToListOfObjects(supportresult[0].Data);
              await dataProvider.setSupportData(supportData);
            }
            setState(() {
              isDataLoaded = true;
              navigateToSplash = false;
 ...
 Future<List<ApiResponse>> SupportMenu() async {
    ApiResponse dataFuture= new ApiResponse();
    dataFuture = await GenericService.getData(
        url: "TSBE/User/getUserMenuForMobile",
        hashMapBody: {'IsSupportMenu': 'true'},
        request: 'GET');
        //print(dataFuture);
    return [dataFuture];
  ...
  Widget build(BuildContext context) {
    // print('Another ${supportData}');
    // print( ' SupportData is ${rolesData}');
