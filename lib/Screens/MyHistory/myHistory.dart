...
class _MyHistoryState extends State<MyHistory> with TickerProviderStateMixin {
  ...
  Future<List<ApiResponse>> APICalls(userProvider, dateTime) async {
    //  final now = DateTime.now().month - 2;
    attendanceHistoryDataFuture =
      ...
      // DateTime(dateTime.threeMonthsDateTime.end.year, dateTime.threeMonthsDateTime.end.month - 2, 1).toString(),
  ...
  Future<void> setTabs(DateTimeRangeProvider dateTime) async {
    tabsList = ['Jan', 'Feb', 'Mar'];
    var numberOfMonthsBack = 3;
    var monthNumber = dateTime.dateTime.start!.month;
    var monthIdx = monthNumber - 3;
    if (monthIdx == -2) {
      tabsList = ['Nov', 'Dec', 'Jan'];
    } else if (monthIdx == -1) {
      tabsList = ['Dec', 'Jan', 'Feb'];
    } else {
      for (var idx = 0; idx < 3; monthIdx++) {
        tabsList[idx] = AppConst.months[monthIdx];
        idx++;
      }
    }

    // var m = dateTime.dateTime.start!.month - (numberOfMonthsBack - 1);
    // var m2 = m < 0 ? 12 + m : m;
    // // m2 < 10 ? '0${m2}' : '${m2}';
    // tabsList[0] = AppConst.months2[dateTime.dateTime.start!.month]!;
    // tabsList[1] = AppConst.months[m2 - 2];

    // tabsList = [];
    // tabsList[0] = AppConst.months[dateTime.dateTime.start!.month - 1];
    // tabsList[1] = AppConst.months[dateTime.dateTime.start!.month - 2];
    // tabsList[2] = AppConst.months[dateTime.dateTime.start!.month - 3];
    ...
    setState(() {
      lastMonthData = attendanceHistoryData.where((elem) {
        return elem['AttendanceDate'].toString().contains(
            ...
            (filter == 'All' ? true : elem['Status'] == filter);
      }).toList();
      previousMonthData = attendanceHistoryData.where((elem) {
        return elem['AttendanceDate'].substring(0, 7) ==
                Jiffy()
                    .subtract(months: 1)
                    .startOf(Units.MONTH)
                    .format('yyyy-MM') &&
                                ...
                                child: Column(
                                  children: [
                                    for (var i = 0;
                                      ...
                                      Slice(
                                        height: 120.0,
                                        statusColor: AppConst.statusColors[
