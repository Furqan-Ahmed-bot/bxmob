...
class GenericService {
  static Future<ApiResponse> getData(
    ...
    try {
      ...
      if (request == 'POST') {
        Uri dataURL;
        if (hashMapBody.runtimeType.toString() == 'List<Map<String, String>>' ||
            hashMapBody.runtimeType.toString() ==
                'List<Map<String, dynamic>>') {
          dataURL = Uri.parse('${prefs.getString('ApiUrl')}/${url}');
        } else {
          ...
          dataURL = Uri.parse('${prefs.getString('ApiUrl')}/${url}')
              .replace(queryParameters: hashMapBody);
        ...
        final response = await http.post(dataURL,
            headers: hashMapHeader, body: jsonEncode(hashMapBody));
        ...
        var myurl = Uri.parse('${prefs.getString('ApiUrl')}/${url}')
            .replace(queryParameters: hashMapBody);
        // myurl.queryParameters={};
        // dataURL =
        ...
        //      path: prefs.getString('ApiUrl'),
        //     queryParameters: hashMapBody);
        final response = await http.get(myurl, headers: hashMapHeader);
        print('Token ${hashMapHeader}');
        switch (response.statusCode) {
          case 200:
